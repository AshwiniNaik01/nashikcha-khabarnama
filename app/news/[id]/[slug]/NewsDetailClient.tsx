...
import { Calendar, User, Link as LinkIcon, Check, Play } from "lucide-react";
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-10 font-marathi relative">
          ...
          <div className="flex flex-wrap items-center justify-between gap-6 border-y border-gray-100 py-6 mb-8">
            <div className="flex items-center gap-6 text-gray-600 text-sm md:text-base">
              <div className="flex items-center gap-2">
                ...
                <span className="font-bold text-gray-900">{news.reporterName || "प्रतिनिधी"}</span>
              </div>
              <div className="flex items-center gap-2">
            ...
            <div className="hidden md:flex items-center gap-2">
              <a href={`https://www.facebook.com/sharer/sharer.php?u=${shareUrl}`} target="_blank" className="w-10 h-10 bg-[#1877F2] text-white flex items-center justify-center rounded-md hover:opacity-90 transition-opacity">
                <svg width="20" height="20" viewBox="0 0 24 24" fill="white"><path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z" /></svg>
              </a>
              <a href={`https://wa.me/?text=${encodeURIComponent(news.title + " " + shareUrl)}`} target="_blank" className="w-10 h-10 bg-[#25D366] text-white flex items-center justify-center rounded-md hover:opacity-90 transition-opacity">
              ...
              <button onClick={copyToClipboard} className={`flex items-center gap-2 px-2 py-2 rounded-full text-xs font-bold transition-all shadow-sm ${copied ? "bg-green-600 text-white" : "bg-gray-100 text-gray-800 hover:bg-gray-200"}`}>
                {copied ? <Check size={25} /> : <LinkIcon size={25} />}
              </button>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-12 gap-8 lg:gap-12">
          ...
          <div className="col-span-12 lg:col-span-8 space-y-10">
            ...
            <article className="prose prose-lg max-w-none prose-p:text-gray-800">
              ...
              <div className="rich-content text-gray-800 leading-relaxed space-y-5" dangerouslySetInnerHTML={{ __html: news.content }} />
            </article>
      ...
      <div className="fixed bottom-10 right-6 flex flex-col gap-3 z-50 md:hidden">
        ...
        <a
          ...
          target="_blank"
          className="w-12 h-12 bg-[#1877F2] text-white flex items-center justify-center rounded-md shadow-lg active:scale-90 transition-transform"
        >
        ...
        <a
          ...
          target="_blank"
          className="w-12 h-12 bg-[#25D366] text-white flex items-center justify-center rounded-md shadow-lg active:scale-90 transition-transform"
        >
