...
const RashiGrid = ({ date }: { date?: string | null }) => {
  ...
  const formatDate = (date: Date) => {
    return date.toLocaleDateString("en-CA"); // YYYY-MM-DD फॉरमॅट
  };

  // २. URL मधील 'date' मधून currentDate मिळवणे
  ...
  // ४. जेव्हा जेव्हा currentDate बदलते, तेव्हा डेटा फेच करणे
  useEffect(() => {
  ...
  if (loading) {
    return (
      <div className="flex flex-col justify-center items-center min-h-[400px] gap-4">
        ...
        <p className="text-gray-500 font-black italic">नशिबाची पाने उलटत आहोत...</p>
      </div>
    );
  }
      ...
      {filteredList.length > 0 ? (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
          {filteredList.map((rashi) => (
            <RashiCard key={rashi._id} rashi={rashi} date={formatDate(currentDate)} />
          ))}
        </div>
      ) : (
        <div className="text-center py-20 bg-white rounded-[2.5rem] border border-dashed border-gray-200">
          ...
          <button
            onClick={() => { setSearchTerm(""); setElementFilter("सर्व"); router.push("/rashi", { scroll: false }); }}
