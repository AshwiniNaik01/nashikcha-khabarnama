...
export default function Navbar() {
  ...
  return (
    <nav className="bg-white sticky top-0 z-[60] shadow-md border-b-2 md:border-b-4 border-lokmat-maroon flex justify-between items-center h-14 md:h-16 px-4">
            ...
            {navItems.map((item) => {
              ...
              return (
                <li key={item.label} className="group relative h-full flex items-center">
                  <Link
                    href={item.href}
                    className={`px-3 xl:px-4 h-full flex items-center gap-1 transition-all duration-300 border-r border-gray-50 whitespace-nowrap ${active ? "bg-lokmat-maroon text-white" : "text-gray-800 hover:bg-lokmat-red hover:text-white"
                      }`}
                  >
                    {item.label}
      ...
      <div
        className={`lg:hidden overflow-y-auto transition-all duration-300 ease-in-out bg-lokmat-maroon ${isOpen ? "max-h-[85vh] opacity-100" : "max-h-0 opacity-0 pointer-events-none"
          }`}
      >
        <ul className="container mx-auto px-4 py-6 flex flex-col gap-1">
          {navItems.map((item) => {
            ...
            return (
              <li key={item.label} className="flex flex-col border-b border-white/5 last:border-0">
                {hasSubItems ? (
                  <>
                    <button
                    ...
                    >
                      {item.label}
                      <FaChevronDown className={`transition-transform duration-300 ${isAdhikOpen ? "rotate-180" : ""}`} />
                    </button>
                    <div
                      className={`grid grid-cols-2 gap-2 pl-4 pb-3 mt-1 overflow-hidden transition-all duration-300 ${isAdhikOpen ? "max-h-96 opacity-100" : "max-h-0 opacity-0 hidden"
                        }`}
                    >
